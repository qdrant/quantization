...
pub fn kmeans(array: &[f32], dim: usize) -> (Vec<f32>, Vec<usize>) {
    match dim {
        1 => kmeans_1d(array),
        2 => kmeans_2d(array),
        _ => unreachable!(),
...
pub fn kmeans_1d(array: &[f32]) -> (Vec<f32>, Vec<usize>) {
    ...
    let mut cluster_size = [0u32; CENTROIDS_COUNT];
    for _ in 0..50 {
        update_indexes_1d(array, &centroids, &mut indexes);
        update_centroids_1d(array, &mut centroids, &indexes, &mut cluster_size);
    }
...
fn update_indexes_1d(array: &[f32], centroids: &[f32; CENTROIDS_COUNT], indexes: &mut [u8]) {
    for (i, &v) in array.iter().enumerate() {
        let mut min_dist = f32::MAX;
        let mut min_index = 0u8;
        for (j, &c) in centroids.iter().enumerate() {
