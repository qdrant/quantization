    ...
    fn score_points(&mut self, points: &[usize], scores: &mut [f32]) {
        unsafe {
                ...
                .zip(scores.chunks_exact_mut(CENTROIDS_COUNT))
            {
                ...
                const SUBVECTOR_SIZE: usize = 16;
                for sv in 0..vector_size / SUBVECTOR_SIZE {
                    for i in 0..CENTROIDS_COUNT {
                        std::ptr::copy_nonoverlapping(
                            self.lut.encoded_vectors.get_ptr(point_ids[i]).add(sv * SUBVECTOR_SIZE),
                            self.points_data
                                .as_mut_ptr()
                                .add(i * SUBVECTOR_SIZE),
                            SUBVECTOR_SIZE,
                        );
                    }
                    ...
                    for _ in 0..SUBVECTOR_SIZE {
                        ...
                        let dists_part = _mm_mullo_epi32(dists_part, alpha2);
                        sum1 = _mm_add_epi32(sum1, dists_part);

                        let dists_part = _mm_srli_epi32(dists, 8);
                        let dists_part = _mm_and_si128(dists_part, distances_mask);
                        let dists_part = _mm_mullo_epi32(dists_part, alpha2);
                        sum2 = _mm_add_epi32(sum2, dists_part);

                        let dists_part = _mm_srli_epi32(dists, 16);
                        let dists_part = _mm_and_si128(dists_part, distances_mask);
                        let dists_part = _mm_mullo_epi32(dists_part, alpha2);
                        sum3 = _mm_add_epi32(sum3, dists_part);

                        let dists_part = _mm_srli_epi32(dists, 24);
                        let dists_part = _mm_and_si128(dists_part, distances_mask);
                        let dists_part = _mm_mullo_epi32(dists_part, alpha2);
                        sum4 = _mm_add_epi32(sum4, dists_part);
                        ...
                        let dists_part = _mm_and_si128(dists, distances_mask);
                        let dists_part = _mm_mullo_epi32(dists_part, alpha1);
                        sum1 = _mm_add_epi32(sum1, dists_part);
                ...
                scores[15] = f32::from_bits(_mm_extract_ps(sum1, 0) as u32) * self.lut.alpha
                    + self.lut.offset;
                scores[11] = f32::from_bits(_mm_extract_ps(sum1, 1) as u32) * self.lut.alpha
                    + self.lut.offset;
                scores[7] = f32::from_bits(_mm_extract_ps(sum1, 2) as u32) * self.lut.alpha
                    + self.lut.offset;
                scores[3] = f32::from_bits(_mm_extract_ps(sum1, 3) as u32) * self.lut.alpha
                    + self.lut.offset;

                scores[14] = f32::from_bits(_mm_extract_ps(sum2, 0) as u32) * self.lut.alpha
                    + self.lut.offset;
                scores[10] = f32::from_bits(_mm_extract_ps(sum2, 1) as u32) * self.lut.alpha
                    + self.lut.offset;
                scores[6] = f32::from_bits(_mm_extract_ps(sum2, 2) as u32) * self.lut.alpha
                    + self.lut.offset;
                scores[2] = f32::from_bits(_mm_extract_ps(sum2, 3) as u32) * self.lut.alpha
                    + self.lut.offset;

                scores[13] = f32::from_bits(_mm_extract_ps(sum3, 0) as u32) * self.lut.alpha
                    + self.lut.offset;
                scores[9] = f32::from_bits(_mm_extract_ps(sum3, 1) as u32) * self.lut.alpha
                    + self.lut.offset;
                scores[5] = f32::from_bits(_mm_extract_ps(sum3, 2) as u32) * self.lut.alpha
                    + self.lut.offset;
                scores[1] = f32::from_bits(_mm_extract_ps(sum3, 3) as u32) * self.lut.alpha
                    + self.lut.offset;

                scores[12] = f32::from_bits(_mm_extract_ps(sum4, 0) as u32) * self.lut.alpha
                    + self.lut.offset;
                scores[8] = f32::from_bits(_mm_extract_ps(sum4, 1) as u32) * self.lut.alpha
                    + self.lut.offset;
                scores[4] = f32::from_bits(_mm_extract_ps(sum4, 2) as u32) * self.lut.alpha
                    + self.lut.offset;
                scores[0] = f32::from_bits(_mm_extract_ps(sum4, 3) as u32) * self.lut.alpha
                    + self.lut.offset;
            }
        }
    }
...
#[cfg(test)]
mod tests {
    use crate::{encoded_vectors::EncodedVectors, simple_scorer::SimpleScorer};
    ...
    fn test_sse_scoring() {
        ...
        let mut encoded = EncodedVectors {
            data: vec![0; count * dim / 2],
            vector_size: dim / 2,
            centroids: vec![], //not needed
            chunks: vec![1; dim],
            dim,
        };
        ...
        for i in 0..count {
            for j in 0..dim / 2 {
                encoded.data[i * dim / 2 + j] = (rng.gen::<u8>() % 16) as u8;
            }
        }

        let mut centroid_distances: Vec<u8> = vec![];
        for i in 0..dim / 2 {
            let alpha1: i32 = 20 * i as i32;
            centroid_distances.extend_from_slice(&alpha1.to_ne_bytes());
            let alpha2: i32 = 20 * i as i32 + 1;
            centroid_distances.extend_from_slice(&alpha2.to_ne_bytes());
            for _ in 0..16 {
                centroid_distances.push(rng.gen::<u8>());
            }
            for _ in 0..16 {
                centroid_distances.push(rng.gen::<u8>());
            }
        ...
        let mut simple_scorer: SimpleScorer = CompressedLookupTable {
        ...
        let mut sse_scorer: SseScorer = CompressedLookupTable {
            encoded_vectors: &encoded,
            centroid_distances: centroid_distances.clone(),
            alpha: 1.0,
            offset: 1.0,
        ...
        let points: Vec<usize> = (0..count).collect();
        let mut scores_simple: Vec<f32> = vec![0.0; count];
        simple_scorer.score_points(&points, &mut scores_simple);

        let mut scores_sse: Vec<f32> = vec![0.0; count];
        sse_scorer.score_points(&points, &mut scores_sse);

        assert_eq!(scores_simple, scores_sse);
    }
}
